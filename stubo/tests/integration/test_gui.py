...
class TestTracker(Base):  
    ...
    def test_session_filter(self):
        ...
        self.http_client.fetch(self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/accept/smart.commands'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)

        self.http_client.fetch(self.get_url('/tracker?session_filter=first_1&start_time=&latency=0&function=all'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
    ...
    def test_latency_filter(self):
        self.http_client.fetch(self.get_url('/stubo/default/execCmds?cmdFile=/static/cmds/demo/first.commands'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
    ...
    def test_ignore200_filter(self):
        ...
        self.http_client.fetch(self.get_url('/tracker?session_filter=&start_time=&latency=0&function=all&show_only_errors=true'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
    ...
    def test_allhosts_filter(self):
        ...
        self.http_client.fetch(self.get_url('/tracker?session_filter=first_1&start_time=&latency=0&function=get%2Fresponse'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        ...
        # all_hosts = true
        self.http_client.fetch(self.get_url('/tracker?session_filter=first_1&start_time=&latency=0&function=get%2Fresponse&all_hosts=true&'), self.stop)
        response = self.wait()
    ...
    def test_function_filter(self):
        ...
        # filter for put/stub 
        self.http_client.fetch(self.get_url('/tracker?session_filter=&start_time=&latency=0&function=put/stub'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.assertFalse('>end/session</a>' in response.body)
        self.assertTrue('>put/stub</a>' in response.body)
    ...
    def test_handles_xml_in_stubo_response(self):
        # start record mode
        self.http_client.fetch(self.get_url('/stubo/api/begin/session?scenario=first&session=first_1&mode=record'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        ...
        # start playback
        self.http_client.fetch(self.get_url('/stubo/api/begin/session?scenario=first&session=first_1&mode=playback'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
...
class TestManage(Base):   
    ...
    def test_run_cmd(self):
        self.http_client.fetch(self.get_url('/manage/exec_cmds?cmdfile=/static/cmds/demo/first.commands&html=true'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
