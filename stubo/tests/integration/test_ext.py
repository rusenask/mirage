...
class TestMangling(Base):
    ...
    def test_roundtrip(self):
        self.http_client.fetch(self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/ext/xslt/first.commands'),
                               self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
...
<dt>2013-11-25</dt>
        ...
        self.assertEqual(track1['stubo_response'],
                         '<response><a>a</a><trans_id>1234</trans_id><b>b</b><user uuid="xxx">joe</user><dt>2013-11-25</dt></response>\n')
        track2 = tracks['2']
...
<dt>2013-11-26</dt>
        ...
        self.assertEqual(track2['stubo_response'],
                         '<response><a>a</a><trans_id>12345</trans_id><b>b</b><user uuid="yyy">mary</user><dt>2013-11-26</dt></response>\n')
...
class TestUserCache(Base):
    ...
    def test_roundtrip(self):
        self.http_client.fetch(self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/ext/cache/cache.yaml'),
                               self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.assertEqual(response.headers["Content-Type"],
...
class TestTextUserCache(Base):
    ...
    def test_roundtrip(self):
        self.http_client.fetch(
            self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/ext/cache/text/1.commands'),
            self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.assertEqual(response.headers["Content-Type"],
...
class TestModule(Base):
    ...
    def test_missing_module_does_raise_error(self):
        self.http_client.fetch(self.get_url('/stubo/api/begin/session?scenario=split1&session=s1&mode=record'),
                               self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
...
class Test_xmlexit(Base):
    ...
    def test_get_response_strip_ns(self):
        self.http_client.fetch(
            self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/ext/get_response_strip_ns/record.commands'),
            self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.http_client.fetch(
            self.get_url('/stubo/api/begin/session?scenario=strip_namespace&session=strip_namespace_1&mode=playback'),
            self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
...
<a>goodbye</a>
    ...
    def test_skip_xml(self):
        self.http_client.fetch(
            self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/ext/auto_mangle/skip_xml/record.commands'),
            self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.http_client.fetch(
            self.get_url('/stubo/api/begin/session?scenario=ignore_xml&session=ignore_xml1&mode=playback'),
            self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
...
<a>goodbye</a>
    ...
    def test_skip_xml_multiple(self):
        self.http_client.fetch(
            self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/ext/'
                         'auto_mangle/skip_xml_multiple/record.commands'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.http_client.fetch(
            self.get_url('/stubo/api/begin/session?scenario=ignore_xml&session=ignore_xml1&mode=playback'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.http_client.fetch(self.get_url('/stubo/api/get/response?session=ignore_xml1'), callback=self.stop,
            method="POST", body="""<user:Request xmlns:user="http://www.my.com/userschema">
...
<a>goodbye</a>
    ...
    def test_skip_xml_elements(self):
        self.http_client.fetch(
            self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/ext/'
                         'auto_mangle/skip_xml_elements/record.commands'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.http_client.fetch(
            self.get_url('/stubo/api/begin/session?scenario=ignore_xml&session=ignore_xml1&mode=playback'), self.stop)
...
<a>goodbye</a>
    ...
    def test_skip_xml_attrs(self):
        self.http_client.fetch(
            self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/'
                         'ext/auto_mangle/skip_xml_attrs/record.commands'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.http_client.fetch(
            self.get_url('/stubo/api/begin/session?scenario=ignore_xml&session=ignore_xml1&mode=playback'), self.stop)
...
<a>goodbye</a>
    ...
    def test_embedded(self):
        self.http_client.fetch(self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/'
                                            'ext/auto_mangle/embedded/record.commands'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.http_client.fetch(
            self.get_url('/stubo/api/begin/session?scenario=embedded&session=embedded_play&mode=playback'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
...
<a>my response is kind</a>
    ...
    def test_embedded_multiple(self):
        self.http_client.fetch(self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/ext/auto_mangle/'
                                            'embedded_multiple/record.commands'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.http_client.fetch(
            self.get_url('/stubo/api/begin/session?scenario=embedded&session=embedded_play&mode=playback'), self.stop)
...
<a>my response is kind</a>
    ...
    def test_all(self):
        self.http_client.fetch(
            self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/ext/auto_mangle/all/record.commands'),
            self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.http_client.fetch(self.get_url('/stubo/api/begin/session?scenario=all&session=all_play&mode=playback'),
                               self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
...
<user:dispatchTime>
    ...
    def test_response(self):
        self.http_client.fetch(
            self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/ext/auto_mangle/response/record.commands'),
            self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.http_client.fetch(
            self.get_url('/stubo/api/begin/session?scenario=response&session=response_play&mode=playback'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.http_client.fetch(self.get_url('/stubo/api/get/response?session=response_play&getresponse_arg=foo'),
                               callback=self.stop,
...
<user>fred</user>
        ...
        self.http_client.fetch(self.get_url('/stubo/api/get/response?session=response_play&played_on=2014-12-15'),
                               callback=self.stop,
                               method="POST",
                               body="""<request2>
<dt>2014-12-25</dt>
<user>fred</user>
    ...
    def test_response_multiple(self):
        self.http_client.fetch(
            self.get_url('/stubo/api/exec/cmds?cmdfile=/static/cmds/tests/ext/auto_mangle/'
                         'response_multiple/record.commands'), self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.http_client.fetch(
            self.get_url('/stubo/api/begin/session?scenario=response&session=response_play&mode=playback'), self.stop)
        ...
        self.assertEqual(response.code, 200)
        self.http_client.fetch(
            self.get_url('/stubo/api/get/response?session=response_play&getresponse_arg=foo&played_on=2014-12-15'),
            callback=self.stop,
            method="POST",
            body="""<request>
<dt>2014-12-25</dt>
...
class TestUnicode(Base):
    ...
    def test_non_ascii_matcher_and_response(self):
        self.http_client.fetch(self.get_url('/stubo/api/exec/cmds?cmdFile=/static/cmds/tests/ext/unicode/all.commands'),
                               self.stop)
        response = self.wait()
        self.assertEqual(response.code, 200)
        self.assertEqual(response.headers["Content-Type"],
