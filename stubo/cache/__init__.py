...
(Hash)
name                            key->value (json)
host:scenario_name:request      session_name:request_id->[[response_ids], delay_policy_name, recorded, system_date, module_info, request_index_key]
...
2) "[[\"1a90f47bb0af291264a6c06868b97cd62b372d41de26c3fd21cef61b\"], \"\", \"2014-11-21\", \"2014-11-25\", {}, \"84dab03cd9cbf56782635f95ef74efc641d993e768b79b4344452b45\"]"
...
class Cache(object):
    ...
    def get_sessions_status(self, scenario_name, status=None, local=True):
        ...
        sessions = [(x[1]['session'], x[1]['status']) for x in \
                    self.get_sessions(scenario_name, 
                                      local=local) if x[1]['status'] in status]
        return sessions 
...
def add_request(session, request_id, stub, system_date, stub_number, 
    ...
    if len(cached_requests) < request_cache_limit:
        ...
        log.debug('add_request: {0} {1} {2} {3} {4} stub_number={5} '\
            ...
            stub.delay_policy_name(), stub_number, request_index_key, result))         
