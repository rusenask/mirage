...
class TestFunctions(unittest.TestCase):
    ...
    def test_get_tracks_with_args(self):
        ...
        response = get_tracks(DummyRequestHandler(), 'mangle', True, 50, 1000, 
                dt.strftime('%Y-%m-%d %H:%M:%S'), 200, True, 'get/response')
        self.assertEqual(response['_filter'],  {'function': 'get/response', 
          'request_params.session' : {'$regex': 'mangle'}, 'start_time': {'$lt': dt},
          'duration_ms': {'$gt': 200}, 'return_code' : { '$ne': 200}})                         
